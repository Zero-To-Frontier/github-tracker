...
생태계의 [단계별 탐색](docs/ecosystem-drilldown-plan.md)은 **전체 지도 → MCP → TypeScript SDK** 시범 경로를 구현했습니다. 한 번 클릭하면 기존처럼 역할과 관계를 확인하고, 더블클릭하거나 `세부지도 보기`를 누르면 새로운 연관 요소가 펼쳐집니다. 다른 대표 항목과 분야 탭으로의 확대는 후속 범위입니다.
...
- AI 생태계(`/ecosystem`): 역할별 지도, 이름·역할 검색, 선택한 항목의 방향성 관계, 데스크톱 상세 패널과 모바일 상세 시트. MCP·TypeScript SDK의 세부 지도 진입·펼침 애니메이션, 탐색 경로와 뒤로가기를 제공.
- 모바일 화면, 키보드 탐색, 프로젝트별 직접 URL과 메타데이터.

## AI 생태계 지도와 콘텐츠 편집

[AI 생태계 지도](http://127.0.0.1:3000/ecosystem)의 **전체 화면은 12개 영역·52개 항목·79개 관계**를 유지합니다. **2026-09-14** 공식 자료를 확인한 정적 편집 자료이며, 생성형 AI·에이전트의 역할을 설명하는 대표 항목을 선정했습니다. 시장 규모·사용량·스타 순위나 AI 전체의 완전한 목록을 뜻하지 않습니다. 기존 추적 저장소 10개에 제한하지 않고 서비스와 공개 표준·공개 가중치 모델, GPU 기반 소프트웨어도 포함합니다. 전통적 머신러닝·로보틱스·미디어 제작 제품 전체는 이번 범위에 포함하지 않습니다.

MCP와 TypeScript SDK 세부 지도는 각각 **중앙 포함 10개 항목·14개 관계·4개 역할 그룹**을 보여줍니다. 세부 탐색 전용으로 **12개 항목·20개 관계**를 추가해 공유 데이터는 총 **64개 항목·99개 관계**이며, 이 전체를 첫 화면에 노출하지 않습니다. MCP에서는 공식 SDK·전송 규약·검사 도구 등을, TypeScript SDK에서는 새로 등장하는 Zod·Express·Tools·Resources·Prompts를 확인합니다. 여러 장면의 공통 항목은 동일 ID와 설명을 재사용합니다.

한 번 클릭은 관계 강조와 상세 확인을 유지합니다. 준비된 항목을 더블클릭하거나 상세의 `세부지도 보기`를 누르면 선택한 항목을 중심으로 연결선과 카드가 펼쳐집니다. `뒤로가기`와 상단 탐색 경로는 이전 지도의 선택·위치·포커스를 복원합니다. 모바일·키보드에서는 버튼으로 같은 경로를 이용하며, 동작 줄이기 설정을 코드에서 지원합니다.

데스크톱 1440·1280px와 모바일 390·360px에서 클릭·더블클릭·버튼 진입, MCP → TypeScript SDK → 뒤로가기, 전체 지도 복귀, 선택·스크롤·포커스 복원을 확인했습니다. Enter·Escape, 모바일 상세 시트의 포커스 제한, 가로 넘침과 관계 라벨 겹침도 검증했습니다. 상세 기록은 [디자인 QA](design-qa.md)에 있으며 동작 줄이기 설정의 브라우저 시뮬레이션은 수행하지 않았습니다.
...
| [data/ecosystem-drilldown.ts](data/ecosystem-drilldown.ts) | 세부 지도 전용 항목·관계와 공식 근거 |
...
| [lib/ecosystem-scenes.ts](lib/ecosystem-scenes.ts) | 공유 데이터, 장면별 노출 목록·역할 그룹·진입 경로 |
| [lib/ecosystem-navigation.ts](lib/ecosystem-navigation.ts) | 단계별 탐색과 이전 상태 복원 |
...
| [tests/ecosystem-scenes.test.ts](tests/ecosystem-scenes.test.ts) | 장면 참조·신규 발견·공유 항목·출처 검증 |
...
세부 지도는 `ecosystem-scenes.ts`의 장면별 노출 목록으로 구성합니다. 신규 항목을 전체 지도 데이터에 추가하지 않아도 되며, 장면의 관계는 실제로 보이는 항목끼리만 연결합니다. 개념·규약에는 실행 소프트웨어의 라이선스를 부여하지 않고 구현체와 구분합니다.

편집 후 `npm test`, `npm run typecheck`, `npm run build`로 확인합니다. 현재 수집·집계·생태계·장면·탐색 상태 테스트 **27개**, 타입 검사와 프로덕션 빌드가 통과했습니다. `npm run collect`는 생태계 편집 데이터를 변경하지 않으며, 수정 사항은 다음 웹 빌드에 반영됩니다. 트래킹·뉴스·논문과의 깊은 연계와 자동 관계 갱신은 후속 범위입니다.
...
- 검증한 MCP·TypeScript SDK 시범 구조를 다른 대표 항목으로 확대하고, 콘텐츠가 준비된 분야 탭 추가.

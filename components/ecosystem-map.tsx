...
import { ArrowRight, ArrowUpRight, BookOpen, Box, Braces, Check, ChevronRight, CircleHelp, Compass, Database, ExternalLink, GitBranch, Github, Globe, Layers3, Map as MapIcon, MemoryStick, Network, RotateCcw, Search, Server, Sparkles, Terminal, Workflow, X } from "lucide-react";
...
const nodeById = new Map(ecosystemNodes.map(node => [node.id, node]));
...
export function EcosystemMap() {
  ...
  const connections = useMemo(() => selectedId ? getConnections(selectedId) : [], [selectedId]);
  const connectedIds = useMemo(() => new Set(connections.map(connection => connection.node.id)), [connections]);
  const searchResults = useMemo(() => {
    const normalized = query.trim().toLocaleLowerCase();
    return normalized ? ecosystemNodes.filter(node => `${node.name} ${node.mapLabel} ${node.tagline} ${node.description} ${categoryById.get(node.category)!.name}`.toLocaleLowerCase().includes(normalized)) : [];
  }, [query]);
  ...
  const close = useCallback(() => {
    ...
    setSelectedId(null);
    if (previous) requestAnimationFrame(() => nodeRefs.current.get(previous)?.focus());
  }, [selectedId]);

  const select = (id: string) => { setSelectedId(id); setQuery(""); };
  ...
  return <main id="main" className={styles.page}>
    <div inert={mobile && !!selected ? true : undefined}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.viewLabel}><MapIcon size={17} /><strong>생태계 지도</strong><span>{ecosystemCategories.length}개 영역 <i /> {ecosystemNodes.length}개 항목</span></div>
        <div className={styles.searchWrap}>
          <Search size={16} aria-hidden="true" />
          <input aria-label="생태계 항목 검색" placeholder="이름이나 역할로 찾기" value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && searchResults[0]) select(searchResults[0].id); if (event.key === "Escape") setQuery(""); }} aria-controls={query.trim() ? "ecosystem-search-results" : undefined} />
    ...
    <div className={styles.workspace}>
      <section className={styles.mapSurface} aria-label="AI 생태계 관계 지도" inert={mobile && !!selected ? true : undefined}>
        <div className={styles.mapTop}><span><span className={styles.liveDot} />{selected ? <><strong>{selected.name}</strong>의 연결 {connections.length}개</> : "전체 구조를 둘러보세요"}</span><button onClick={() => { setQuery(""); close(); }} disabled={!selected && !query}><RotateCcw size={13} /> 전체 보기</button></div>
        <div ref={boardRef} className={`${styles.board} ${selected ? styles.boardActive : ""}`} data-testid="ecosystem-board">
          ...
          {ecosystemCategories.map(category => {
            ...
            return <section key={category.id} className={styles.area} style={colors(category.color)} aria-labelledby={`area-${category.id}`}><div className={styles.areaHeader} data-map-area-heading><div><Icon size={17} /><span>{ecosystemNodes.filter(node => node.category === category.id).length}개 항목</span></div><h2 id={`area-${category.id}`}>{category.name}</h2><p>{category.subtitle}</p></div><div className={styles.nodes}>{ecosystemNodes.filter(node => node.category === category.id).map(node => {
              ...
              return <button key={node.id} ref={element => { if (element) nodeRefs.current.set(node.id, element); else nodeRefs.current.delete(node.id); }} className={`${styles.node} ${isSelected ? styles.selected : ""} ${connected ? styles.connected : ""} ${dimmed ? styles.dimmed : ""}`} onClick={() => select(node.id)} aria-pressed={isSelected} aria-label={`${node.name} 상세 보기`} aria-describedby={`role-${node.id}`} aria-controls="ecosystem-detail" data-node-id={node.id}><NodeIcon node={node} /><span className={styles.nodeName}>{node.name}</span><small className={styles.nodeRole} id={`role-${node.id}`}>{node.mapLabel}</small>{isSelected ? <Check size={13} /> : <ChevronRight size={12} />}</button>;
            })}</div></section>;
          })}
        </div>
      ...
      <div id="ecosystem-detail" ref={panelRef} className={`${styles.inspector} ${selected ? styles.inspectorOpen : ""}`} role={mobile && selected ? "dialog" : "complementary"} aria-modal={mobile && selected ? true : undefined} aria-labelledby={selected ? "ecosystem-detail-heading" : "ecosystem-guide-heading"} onKeyDown={trapFocus}>
        {selected ? <>
          ...
          <div className={styles.guideHero}><div className={styles.guideIllustration} aria-hidden="true"><div><Sparkles size={23} /></div><span /><div><Workflow size={23} /></div><span /><div><Database size={23} /></div></div><h2 id="ecosystem-guide-heading">하나의 도구에서,<br />생태계 전체로.</h2><p>궁금한 항목을 선택하면 어떤 역할을 하는지, 무엇과 연결되는지 볼 수 있어요.</p></div>
          ...
          <div className={styles.startingPoints}><h3>여기서 시작해 보세요</h3>{["opencode", "hermes", "mcp"].map(id => { const node = nodeById.get(id); return node && <button key={id} onClick={() => select(id)}><NodeIcon node={node} /><span>{node.name}<small>{categoryById.get(node.category)!.name}</small></span><ArrowUpRight size={15} /></button>; })}</div>
    ...
    <div className={styles.footnote} inert={mobile && !!selected ? true : undefined}><span><Check size={13} />{ecosystemEdges.length}개 연결 · 공식 문서와 저장소를 근거로 편집</span><span>선정한 대표 항목의 지도이며, 전체 목록이나 인기 순위가 아닙니다.</span></div>
  </main>;
}
